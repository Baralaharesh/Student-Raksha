...
st.set_page_config(
    page_title="Raksha AP Cyber Command V5",
    page_icon="🚔",
    layout="wide",
...
# --- HEADER ---
st.title("🚔 RAKSHA AP CYBER COMMAND CENTER V5.0")
st.markdown("### AP State Live Intelligence | AI Chat Bot + Phishield")
st.markdown("---")
...
with col1:
    ...
    for idx, row in map_data.iterrows():
        ...
        folium.CircleMarker(
            ...
            fill_color=color, fill_opacity=0.7, weight=2
        ).add_to(m)
        folium.Marker(
            [row["Lat"], row["Lon"]],
...
with tab2:
    ...
    if st.button("🔍 Scan Now", use_container_width=True):
        if url_to_check:
            # Simple Phishield Logic
            suspicious_keywords = ['kyc', 'update', 'verify', 'bank', 'prize', 'lottery', 'free', 'gift', 'bit.ly', '.xyz', '.tk']
            ...
            if is_suspicious or not url_to_check.startswith("https"):
                ...
                **Reasons:**
                ...
                """)
                st.image("https://i.imgur.com/8Qf8w1g.png", caption="Scam Link Example") # Generic warning image
            else:
